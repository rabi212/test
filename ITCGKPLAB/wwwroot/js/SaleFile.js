...
$(document).ready(function () {
	...
	$('#CreditAmtX').text($('#CreditAmt').val());
	if ($('#SSId').val() == null || $('#SSId').val() == 0) {
		//GrandTotalAmtCount();
		$('#SDate').val(CurrentDate());
	}
	$('#SSVNoX').text($('#SSVNo').val());
	...
	//$('#SaleDetailViewModels_' + $('#RowId').val() + '__OpnItemName').tablecomplete({	
	$('.OpenItemDetailRecord').tablecomplete({
		//position: { my: "left bottom", at: "right bottom"},
		minLength: 1,
		source: projects,
		select: function (event, ui) {
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__ItemCode').val(ui.item.id);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__SSItemName').val(ui.item.value);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__GSTPer').val(ui.item.gstpercentage);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__UnitCase').val(ui.item.unitcase);
			// $('#SaleDetailViewModels_' + $('#RowId').val() + '__CessPer').val(ui.item.cessrate);
			return false;
		}
	});
...
function FilterItemBatchNoAutoAdd() {
	if ($('#SaleDetailViewModels_' + $('#RowId').val() + '__RecordType').val() != "Old") {
		$.ajax({
			url: "/Financial/GetItemBalanceRecordAdd",
			method: "GET",
			data: { itemid: $('#SaleDetailViewModels_' + $('#RowId').val() + '__ItemCode').val(), cmpid: $("#CompId").val() },
			success: function (result) {
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__ExpDate').val(result.expDate);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__BatchNo').val(result.batchNo);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__PurRate').val(result.purRate);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__NetPurRate').val(result.netPurRate);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__GSTPer').val(result.gstPer);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__MRP').val(result.mrp);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__SaleRate').val(result.saleRate);
				$('#SaleDetailViewModels_' + $('#RowId').val() + '__CustSaleRate').val(result.saleRate);
			},
			error: function (errormessage) {
				console.error(errormessage.responseText);
			}
...
function BatchSearchRecord() {
	$('.BatchDetailRecord').tablecompleteB({
		...
		source: projectsbat,
		select: function (event, ui) {
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__ExpDate').val(ui.item.id);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__BatchNo').val(ui.item.value);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__PurRate').val(ui.item.purrate);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__NetPurRate').val(ui.item.netpurrate);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__GSTPer').val(ui.item.gstper);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__MRP').val(ui.item.mrp);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__SaleRate').val(ui.item.salerate);
			$('#SaleDetailViewModels_' + $('#RowId').val() + '__CustSaleRate').val(ui.item.salerate);
			return false;
		}
	});
}
...
function TotalSaleAmt() {
	var opnCase = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__Qty').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__Qty').val().trim() == "" ? 0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__Qty').val();
	var caserate = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__CustSaleRate').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__CustSaleRate').val().trim() == "" ? 1 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__CustSaleRate').val();

	var totalamt = parseFloat((parseFloat(opnCase) * parseFloat(caserate))).toFixed(2);
	$('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val(totalamt);
}
function TotalDiscountAmt() {	
	var discAmt1 = 0;
	var puramt = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val().trim() == "" ? 0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val();
	var discper1 = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__DiscPer1').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__DiscPer1').val().trim() == ""  ? 0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__DiscPer1').val();
	if (parseFloat(discper1).toFixed(2) > 0) {
		discAmt1 = parseFloat(parseFloat(puramt) * parseFloat(discper1) * 0.01).toFixed(2);
	}
	else {
		discAmt1 = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val().trim() == "" ? 0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val();
	}
	$('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val(discAmt1);
}
function NetSaleAmount() {	
	var discAmt1 = 0; var discAmt2 = 0; var discAmt3 = 0;
	discAmt1 = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val()) == true || $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val().trim() == "" ? 0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalDiscAmt').val();
	var Saleamt = isNaN($('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val()) == true  || $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val().trim() == "" ?  0 : $('#SaleDetailViewModels_' + $('#RowId').val() + '__TotalAmt').val();
	
	var netsaleamt = parseFloat(parseFloat(Saleamt) - parseFloat(discAmt1)).toFixed(2);	
	
	$('#SaleDetailViewModels_' + $('#RowId').val() + '__NetTotalAmt').val(netsaleamt);
}
function GrandTotalAmtCount() {
	...
	for (var i = 0; i < rowscount; i++) {
		...
		TotalSaleAmt = parseFloat(TotalSaleAmt) + parseFloat($('#SaleDetailViewModels_' + i + '__TotalAmt').val());
		Totaldisc = parseFloat(Totaldisc) + parseFloat($('#SaleDetailViewModels_' + i + '__TotalDiscAmt').val());
		TotalNetAmt = parseFloat(TotalNetAmt) + parseFloat($('#SaleDetailViewModels_' + i + '__NetTotalAmt').val());
		netamt = parseFloat($('#SaleDetailViewModels_' + i + '__NetTotalAmt').val());
		gstrate = parseFloat($('#SaleDetailViewModels_' + i + '__GSTPer').val());
	...
	var creditAmt = isNaN($('#CreditAmt').val()) == true || $('#CreditAmt').val().trim() =="" ? 0 : $('#CreditAmt').val();
	var netamt = parseFloat(parseFloat(TotalNetAmt) - parseFloat(creditAmt)).toFixed(2);
	$('#PaidAmt').val(parseFloat(netamt).toFixed(2));	
}
function FinalTotalSaleAmt() {
	var TotalAmt = isNaN($('#NetAmt').val()) == true ? 0 : $('#NetAmt').val();
	var creditAmt = isNaN($('#CreditAmt').val()) == true || $('#CreditAmt').val().trim() == "" ? 0 : $('#CreditAmt').val();

	var netamt = parseFloat(parseFloat(TotalAmt) - parseFloat(creditAmt)).toFixed(2);
	$('#PaidAmt').val(parseFloat(netamt).toFixed(2));	
}
function GetPatientOldNew() {
	if ($('#SSId').val() == null || $('#SSId').val() == 0) {
		$('#CustName').val(''); $('#CustAddress1').val('');
		$('#DrName').val(''); $('#CreditAmt').val('0.00');
		$('#CreditAmtX').text('0.00');
		$.ajax({
			...
			success: function (Data) {
				...
				$('#CreditAmt').val(Data.balAmt);
				$('#CreditAmtX').text(Data.balAmt);
			},
			error: function (errormessage) {
				console.error(errormessage.responseText);
			}
...
function GetPurVoucherSrNo() {
	if ($('#SSId').val() == null || $('#SSId').val() == 0) {
		$.ajax({
			url: "/Transaction/GetSaleFileSrNo/",
			method: "GET",
			data: { cmpId: $('#CompId').val() },
			success: function (Data) {
				$('#SSVNo').val(Data);
...
$("#btnAdd").on('click', function () {
	$.ajax({
		...
		type: "POST",
		url: '/Transaction/AddOrderSaleFileItem',
...
function deleteRow(element) {
	...
	if (message) {
		$.ajax({
			async: true,
			data: $('#saleform').serialize(),
			url: "/Transaction/DeleteOrderSaleFileItem",
			type: "Post",
		})
